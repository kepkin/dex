...
func testTotpCRUD(t *testing.T, s storage.Storage) {
	// Use bcrypt.MinCost to keep the tests short.
	secretHash1, err := bcrypt.GenerateFromPassword([]byte("secret"), bcrypt.MinCost)
	if err != nil {
		t.Fatal(err)
	}
	...
	getAndCompare := func(id string, want storage.TotpSecret) {
		gr, err := s.GetOtp(id)
		if err != nil {
			t.Errorf("get totp %q: %v", id, err)
			return
		}
		if diff := pretty.Compare(want, gr); diff != "" {
